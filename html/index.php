...
function newReplyTweet($tweet_textarea)
{
  
  createReplyTweet($tweet_textarea, $_SESSION['user_id'], $_POST['reply_id']);
}
...
if ($_POST) { /* POST Requests */
    if (isset($_POST['logout'])) { //ログアウト処理
        ...
        newReplyTweet($_POST['tweet_textarea']);
      ...
      <div class="card-body">
        <form method="POST">
          ...
          <input type="hidden" name="reply_id" value="<?= $_GET['reply'] ?>" />
           <?php } else { //返信フラグが立っていない場合、テキストエリア＝空白、隠し項目なし?>
